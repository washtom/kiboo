...
contract  desireLottery is owned {
    ...
    uint public maxUserProof = 64;      //最多64位参与者
    ...
    uint public toUserPer100 =  88;     //返奖率，高于75%
    uint public toAgentPer100 = 10;    //给代理，低于15%
    uint public toOwnerPer100 =  2;     //自己利润，所有剩下的。最低1%。
        ...
        private returns(bool _result)
    {
       ...
       currentRandomByte32 = sha3(currentRandomByte32, block.blockhash(block.number), now + currentPeriodIndex);
        ...
        userBalanceOf[_buyer] -= buyAmount; 
        userBalanceOf[this] += buyAmount * toUserPer100 / 100;              //2，奖金金额放到this名下。
        if(_isByAgent){ 
    ...
    event UserWin(address indexed caller, address indexed user, uint multiple, uint okAmount);  //中奖通知事件
    ...
    function shareOkAmount(uint _okCode, uint _okAmount) private {
        for (uint m = 0; m < currentPeriodIndex; m++){
            ...
            if(multiple > 0){
                ...
                UserWin(msg.sender, thisus.user, multiple, _okAmount);   //时间，谁中奖了！
            }
        }
        return;
    }
    ...
    function canOpenLottery() private returns(bool _result){  
        if(currentPeriodIndex > maxUserProof){         //数量到了最多
            return true;
        }
        if (now >= currentPeriodEnd)                    //时间到了
        {
    ...
    function openLottery()  public returns(bool _result)
    {
        if(canOpenLottery())   
        {
            ...
            shareOkAmount(okCode, okAmount);   //分奖金
    ...
    //uint256 constant  openLotteryMsg = 0.000096 ether;
    uint256 public  constant refundCode = 88;//      0.000088 ether;
    uint256 public  constant openLotteryCode = 98;  // 0.000098 ether;
    ...
    function _userRefund(address _to) private  returns(bool _result){
        ...
        userBalanceOf[msg.sender] -= _balanceAmount;            //userBalanceOf[msg.sender] = 0;

        require(userBalanceOf[msg.sender] >= 0);         // == 0            
        _to.transfer(_balanceAmount);                    //和外部有交互，防范攻击！
        return true;
    }
